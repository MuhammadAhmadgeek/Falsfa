      ...
      <footer className="border-t border-white/5 py-16 px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-4 gap-10 mb-12">
            ...
            {[
              { title: 'Product', links: ['Features', 'Modules'] },
              { title: 'Company', links: ['About', 'Contact'] },
              { title: 'Legal', links: ['Privacy', 'Terms'] },
            ].map(g => (
              <div key={g.title}>
                <h4 className="text-xs font-bold text-white/50 uppercase tracking-[0.2em] mb-4">{g.title}</h4>
                <ul className="space-y-2.5">{g.links.map(l => <li key={l}><a href="#" className="text-sm text-white/25 hover:text-white/50 transition-colors duration-300">{l}</a></li>)}</ul>
              </div>
            ))}
          </div>
